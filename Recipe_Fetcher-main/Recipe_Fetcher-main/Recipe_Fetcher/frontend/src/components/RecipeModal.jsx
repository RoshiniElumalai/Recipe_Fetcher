...
const RecipeModal = ({ recipeId, onClose, userIngredients = [] }) => {
  ...
  const printRecipe = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>${recipe.title} - Man's Food Guide</title>
          <style>
            body { font-family: Arial, sans-serif; padding: 20px; max-width: 800px; margin: 0 auto; }
            h1 { color: #3E5F44; border-bottom: 2px solid #3E5F44; padding-bottom: 10px; }
            .info { background: #E8FFD7; padding: 15px; border-radius: 10px; margin: 20px 0; }
        ...
        <body>
          ...
          <p class="no-print"><br>Printed from Man's Food Guide - Cook with what you have!</p>
        </body>
      </html>
    `)
  ...
  const shareRecipe = async () => {
    ...
    const shareData = {
      title: recipe.title,
      text: `Check out this recipe from Man's Food Guide: ${recipe.title}`,
        ...
        <div className="flex justify-between items-start p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <div className="flex items-center gap-4 text-gray-600 dark:text-gray-400">
              ...
              <span className="text-sm px-2 py-1 bg-[#E8FFD7] dark:bg-[#3E5F44]/30 text-[#3E5F44] dark:text-[#93DA97] rounded-full">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Heart size={20} className={isFavorite ? "fill-[#3E5F44] text-[#3E5F44]" : ""} />
            </button>
            <button
              onClick={printRecipe}
              className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full"
              ...
              <div className="border-b border-gray-200 dark:border-gray-700 mb-6">
                <div className="flex space-x-4">
                  <button
                    ...
                    className={`pb-2 px-1 font-medium ${activeTab === 'ingredients' ? 'border-b-2 border-[#3E5F44] dark:border-[#5E936C] text-[#3E5F44] dark:text-[#5E936C]' : 'text-gray-500'}`}
                  >
                    Ingredients
                  </button>
                  <button
                    onClick={() => setActiveTab('instructions')}
                    className={`pb-2 px-1 font-medium ${activeTab === 'instructions' ? 'border-b-2 border-[#3E5F44] dark:border-[#5E936C] text-[#3E5F44] dark:text-[#5E936C]' : 'text-gray-500'}`}
                  >
                    Instructions
                  </button>
                  <button
                    onClick={() => setActiveTab('nutrition')}
                    className={`pb-2 px-1 font-medium ${activeTab === 'nutrition' ? 'border-b-2 border-[#3E5F44] dark:border-[#5E936C] text-[#3E5F44] dark:text-[#5E936C]' : 'text-gray-500'}`}
                  >
                    Nutrition
                  </button>
                </div>
                      ...
                      return (
                        <div 
                          ...
                          className={`flex items-center gap-3 p-3 rounded-lg ${
                            status === 'have' ? 'bg-[#E8FFD7] dark:bg-[#3E5F44]/20' : 
                            status === 'missing' ? 'bg-red-50 dark:bg-red-900/20' : 
                            'bg-gray-50 dark:bg-gray-700'
                          }`}
                        >
                          <div className={`flex-shrink-0 ${status === 'have' ? 'text-[#3E5F44]' : status === 'missing' ? 'text-red-500' : 'text-gray-400'}`}>
                  ...
                  <div className="space-y-6">
                    {(recipe.analyzedInstructions?.[0]?.steps || []).map((step, index) => (
                      <div key={index} className="flex gap-4">
                        <div className="flex-shrink-0 w-8 h-8 bg-[#3E5F44] dark:bg-[#5E936C] text-white rounded-full flex items-center justify-center font-bold">
                          {index + 1}
                        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 p-6 rounded-2xl">
                ...
                <div className="space-y-4">
                    ...
                    {[1, 2, 4, 6, 8].map(num => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-full flex items-center justify-center font-bold transition-all ${
                          servings === num
                            ? 'bg-[#3E5F44] dark:bg-[#5E936C] text-white scale-110'
                  ...
                  <input
                    ...
                    className="w-full h-2 bg-gray-200 dark:bg-gray-600 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#3E5F44] dark:[&::-webkit-slider-thumb]:bg-[#5E936C]"
                  />
                </div>
              </div>
                  ...
                  {['mild', 'medium', 'spicy'].map(level => (
                    <button
                      ...
                      className={`w-full p-3 rounded-lg flex items-center justify-between transition-colors ${
                        spiceLevel === level
                          ? 'bg-[#3E5F44] dark:bg-[#5E936C] text-white'
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full bg-[#3E5F44] dark:bg-[#5E936C] hover:bg-[#2D4A33] dark:hover:bg-[#4A7B5A] text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2"
                >
                  <ChefHat size={20} />
                  Add to Meal Plan
                </button>
                ...
                <button
                  onClick={printRecipe}
                  className="w-full border-2 border-[#3E5F44] dark:border-[#5E936C] text-[#3E5F44] dark:text-[#5E936C] hover:bg-[#3E5F44] dark:hover:bg-[#5E936C] hover:text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors"
                >
                  <Printer size={20} />
                  Print Recipe
                </button>
              ...
              {/* Quick Tips */}
              <div className="bg-[#E8FFD7] dark:bg-[#3E5F44]/20 p-4 rounded-xl">
                <h4 className="font-bold mb-2 text-[#3E5F44] dark:text-[#93DA97]">Quick Tips</h4>
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-500">
              Recipe from {recipe.creditsText || recipe.source?.replace('_', ' ') || "Man's Food Guide"}
            </div>
            <button
              onClick={onClose}
